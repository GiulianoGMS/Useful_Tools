 SELECT  DECODE(GV$SESSION.SID || GV$SESSION.INST_ID, NULL               , 1, NULL) FIRSTROW, INITCAP(NVL(GV$SESSION.USERNAME, '{Oracle}')), GV$SESSION.SID, GV$SESSION.SERIAL#, INITCAP(GV$INSTANCE.INSTANCE_NAME), GV$SESSION.AUDSID, GV$PROCESS.SPID, INITCAP(GV$SESSION.SERVER), (SELECT ROUND( (VALUE / 100 ) / ROUND( GREATEST( SYSDATE - GV$SESSION.LOGON_TIME, 0.001 ) * 1440 * 60) * 100) FROM GV$SESSTAT SS WHERE SS.INST_ID = GV$SESSION.INST_ID AND SS.SID = GV$SESSION.SID AND 
SS.STATISTIC# = 19),  NVL(( SELECT ROUND(S.VALUE / GREATEST((SELECT SUM(T.VALUE)
                           FROM GV$SESSTAT T
 ...
 WHERE S.STATISTIC# = 19
  ...
  AND S.INST_ID = GV$SESSION.INST_ID ), 0) , ROUND( NVL( GV$PROCESS.PGA_ALLOC_MEM, 0) / 1024 / 1024 ) ,  ROUND( NVL( TMP.USAGE_MB, 0) ) ,  ROUND( NVL( UND.USAGE_MB, 0) ) , CAST(TRIM(TO_CHAR(TRUNC(GV$SESSION.LAST_CALL_ET / 60 / 60))) || ':' ||
        TRIM(TO_CHAR(TRUNC(MOD(GV$SESSION.LAST_CALL_ET, 3600) / 60), '00')) || ':' ||
        TRIM(TO_CHAR(MOD(MOD(GV$SESSION.LAST_CALL_ET, 3600), 60), '00')) AS VARCHAR2(250)) , GV$INSTANCE.INST_ID, INITCAP(DECODE(GV$SESSION.LAST_CALL_ET, 0, 'ACTIVE', GV$SESSION.STATUS)) ,  DECODE( GV$SESSION.BLOCKING_SESSION, NULL, NULL, DECODE( GV$SESSION.BLOCKING_SESSION_STATUS, 'VALID', 'SID ' || GV$SESSION.BLOCKING_SESSION || '.' || GV$SESSION.BLOCKING_INSTANCE, NULL ) ) , INITCAP( GV$SESSION.EVENT || DECODE(DBA_OBJECTS.OBJECT_NAME, NULL, '', ' \ ' || DBA_OBJECTS.OBJECT_NAME) || DECODE(PL.OBJECT_NAME, NULL, '', ' \ ' || PL.OBJECT_NAME ) ), INITCAP(GV$SESSION.OSUSER), INITCAP(GV$SESSION.TERMINAL), INITCAP(GV$SESSION.PROGRAM), GV$SESSION.LOGON_TIME, GV$SESSION.CLIENT_INFO, GV$SESSION.CLIENT_IDENTIFIER, INITCAP(DECODE(
DBA_SCHEDULER_RUNNING_JOBS.JOB_NAME, NULL,
DECODE(DBA_JOBS_RUNNING.JOB, NULL, GV$SESSION.MODULE, 'Job ' || DBA_JOBS_RUNNING.JOB), 'Sch ' || DBA_SCHEDULER_RUNNING_JOBS.JOB_NAME)), INITCAP(GV$SESSION.ACTION), NVL(INFO.CLIENT_VERSION, 'Unknown'), NVL(INFO.CLIENT_CHARSET, 'Unknown')  FROM GV$SESSION, GV$PROCESS, GV$INSTANCE, 
