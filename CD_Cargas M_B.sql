-- Consulta de Status das Cargas do CD (Montadas / Agrupadas)

SELECT ETQPALETECARREG ETIQUETA, DESCTIPESPECIE EMPRESA, TO_CHAR(DTAHORINIMONT, 'DD/MM/YYYY') DATA_MONTAGEM, USUMONTAGEM USUARIO, DESCSTATUSCARREG STATUS

FROM (

SELECT  MIN(XXX.ETQPALETECARREG)||' - '||Min(XXX.ETQPALETECARREGAGRUPAMENTO)         ETQPALETECARREG, 
        MIN(XXX.DESCTIPESPECIE)          DESCTIPESPECIE,
        MIN(XXX.DTAHORINIMONTMASTER)     DTAHORINIMONT,
     /*(CASE WHEN GROUPING(XXX.SEQPALETECARREG) = 1 THEN MIN(XXX.DTAHORFINALMONTAGRUP) Min(XXX.ETQPALETECARREGAGRUPAMENTO)
             ELSE MIN(XXX.DTAHORFINALMONTMASTER) END) DTAHORFINALMONT,*/
        MIN(XXX.USUMONTAGEMMASTER)       USUMONTAGEM,
        MIN(XXX.DESCSTATUSCARREGMASTER)  DESCSTATUSCARREG                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          

FROM  (SELECT  A1.TIPOETQPALETECARREG || A1.SEQPALETECARREG AS ETQPALETECARREGAGRUPAMENTO,
...
AND MLO_PALETECARREG.NROEMPRESA = '501'             
AND TRUNC( MLO_PALETECARREG.DTAHORINIMONT ) >= DATE '2023-04-01'                                      
AND TRUNC( MLO_PALETECARREG.DTAHORINIMONT ) <= DATE '2023-04-02'                                      
AND MLO_PALETECARREG.STATUSCARREG IN ('M','B')

  GROUP BY
    A1.TIPOETQPALETECARREG || A1.SEQPALETECARREG,
    A1.TIPOETQPALETECARREG,
    A1.SEQPALETECARREG,
...
GROUP BY
  SEQPALETECARREGAGRUPAMENTO,
ROLLUP( SEQPALETECARREG )

UNION ALL

SELECT  MLO_PALETECARREG.TIPOETQPALETECARREG || MLO_PALETECARREG.SEQPALETECARREG AS ETQPALETECARREG, 
  MLO_TIPESPECIE.DESCTIPESPECIE,
  MLO_PALETECARREG.DTAHORINIMONT,
  --MLO_PALETECARREG.DTAHORFINALMONT,
  MLO_PALETECARREG.USUMONTAGEM,
