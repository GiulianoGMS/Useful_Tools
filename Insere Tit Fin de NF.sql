...
BEGIN
  SELECT MAX(Z.ROWID)
   ...
   WHERE NUMERODF = 104
     AND NROEMPRESA = 41 
     AND CODGERALOPER = 610;

  PKG_MAD_FATURAMENTO.SP_GERA_MRL_TITULOFIN(vRowId , 'X', 'I');

END;

-- Setar INDREPLICACAO = 'X'
SELECT X.INDREPLICACAO, ROWID FROM MRL_TITULOFIN X WHERE X.NROTITULO = 104 AND NROEMPRESA = 41;
-- Executar Shdl CAFD_INTEG_TIT_FINANCEIRO
-- Ingetrou:
SELECT * FROM FI_TITULO X  WHERE X.NROTITULO = 104 AND NROEMPRESA = 41;
